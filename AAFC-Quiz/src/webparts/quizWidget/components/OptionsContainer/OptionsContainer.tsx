...
import { Checkbox, ChoiceGroup, IChoiceGroupOption, List } from '@fluentui/react';
...
export default class OptionsContainer extends React.Component<IOptionsContainerProps, IOptionsContainerState> {
        ...
        if (item && item.length > 0) {
            return (
                <div style={{ marginBottom: "15px" }}>
                    <Checkbox label={item} disabled={disabled} onChange={this._makeChangeHandler(item)} />
                </div>
            );
        }
    }
    ...
    private _onChange = (ev: React.FormEvent<HTMLInputElement>, option: any): void => {
        this.props.onChange(ev, option, false, this.props.PollId);
    }
    ...
    private _onCheckboxChange = (ev: any, isChecked: boolean, item: string) => {
        ...
        this.props.onChange(ev, { key: finalSel }, true, this.props.PollId);
    }
